...
The animation below is generated by `demo_video_resnet50_baseline_gradcam.py` on 5 sample frames from `imgs/train/c1` (texting — right). Each frame shows the Grad-CAM heatmap overlaid on the full original image at native resolution, with MediaPipe face/hand detections and the `Phone?` heuristic box drawn on top as an independent verification layer.

![Grad-CAM + MediaPipe verification demo](docs/demo_gradcam_combined.gif)
